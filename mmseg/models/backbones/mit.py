...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x, hw_shape, attn_mask=None):
        ...
        def _inner_forward(x, attn_mask=None):
            if attn_mask is not None:
                # import pdb; pdb.set_trace()

                attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
                # if attn_mask.shape[2:] != hw_shape:
                #     attn_mask = resize(attn_mask, size=tuple(hw_shape), mode='bilinear')
...
class MixVisionTransformer(BaseModule):
    ...
    def forward(self, x, no_pos_embed=False, avg_pos_emd=False, duplicate_pos_emd=False, 
        use_fdrop=False, attn_mask=None, attn_mask_weight=0.0, adaptive_attn_mask=False):
        outs = []

        if (attn_mask is not None):
            attn_mask = attn_mask.reshape(attn_mask.size(0), -1)
            if adaptive_attn_mask:
                # if the patch is confident (unconfidence < 0.5), I don't encourage it to be changed
