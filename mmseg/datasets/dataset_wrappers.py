...
@DATASETS.register_module()
class ConcatDataset_SETR(_ConcatDataset):
    """A wrapper of concatenated dataset.
    Same as :obj:`torch.utils.data.dataset.ConcatDataset`, but
    ...
    def __init__(self, datasets):
        super(ConcatDataset_SETR, self).__init__(datasets)
        self.CLASSES = datasets[0].CLASSES
        self.PALETTE = datasets[0].PALETTE

@DATASETS.register_module()
class SemiDataset(ConcatDataset_SETR):
...
@DATASETS.register_module()
class UniSemiDataset(ConcatDataset_SETR):
    """Wrapper for semisupervised od."""

    def __init__(self, sup: dict, unsup: dict, unsup_mix: dict, **kwargs):
        super().__init__([build_dataset(sup), build_dataset(unsup), build_dataset(unsup_mix)], **kwargs)
    @property
    def sup(self):
        return self.datasets[0]
